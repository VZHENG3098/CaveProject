...
public class CaveRoom {
	...
	/**
	 * for every Door in doors[] that is not null, this method appends a String to
	 * "directions" describing the door and where it is. For example: There is a
	 * (passage) to (the North) There is a (passage) to (the East) If there are no
	 * doors that are not null, this sets directions to: "There is no way out. You
	 * are trapped in this room"
	 */
	...
	/**
	 * converts an int to a direction: 0 -> "the North" 1 -> "the East" hint:
	 * complete this method without using an if statement
	 * 
	 * @param dir
	 * @return
	 */
	...
	/**
	 * This is how we join rooms together. It gives this room access to anotherRoom
	 * and vice-versa It also puts the door between both rooms
	 * 
	 * @param direction
	 * @param anotherRoom
	 * @param door
	...
	public static void setUpCaves() {
		...
		for (int row = 0; row < c.length - 1; row++) {
			for (int col = 0; col < c[row].length; col++) {
				if ((row != 1 && row != 4 && row != 3 && row != 2) || col == 0 || col == c[row].length - 1) { 
					//add rows to remove, does not affect first and last columns
	...
	/**
	 * returns the OPPOSITE direction oD(0) returns 2 oD(1) returns 3
	 * 
	 * @param dir
	 * @return
	 */
